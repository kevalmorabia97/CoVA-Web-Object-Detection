...
class WebObjExtractionNet(nn.Module):
    def __init__(self, roi_output_size, img_H, n_classes, backbone='alexnet', context_size=6, hidden_dim=300, trainable_convnet=True,
                 drop_prob=0.2, use_bbox_feat=True, class_names=None):
        """
        Args:
            ...
            context_size: number of BBoxes before and after to consider as context (default: 6)
            hidden_dim: size of hidden contextual representation (default: 300)
        ...
        self.n_total_feat = self.n_visual_feat + self.hidden_dim + self.n_bbox_feat
    ...
    def forward(self, images, bboxes, context_indices):
        ...
        context_outputs = []
        attention_wts = []
        for i in range(2*self.context_size):
        ...
        context_outputs = torch.cat(context_outputs, dim=1).view(batch_size, 2*self.context_size, -1) # [batch_size, 2 * context_size, hidden_dim]
