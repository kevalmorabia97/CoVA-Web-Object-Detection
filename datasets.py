...
class WebDataset(torchvision.datasets.VisionDataset):
    ...
    def __init__(self, root, img_ids, context_size, use_additional_feats=False, sampling_fraction=1):
        ...
        self.all_bboxes = [
            np.loadtxt('%s/bboxes/%s.csv' % (self.root, img_id), delimiter=',', skiprows=1, dtype='float32')
            for img_id in self.ids
        ]
        
        if use_additional_feats:
            self.all_additional_tensor_features = [
                torch.Tensor(np.loadtxt('%s/additional_features/%s.csv' % (self.root, img_id), delimiter=',', skiprows=1, dtype='float32')) 
                for img_id in self.ids
            ]
        else:
